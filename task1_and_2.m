...
for i = 1 : nbr_trials
    ...
    tree_model = fitctree(transpose(X_train),transpose(Y_train)); 
    svm_model = fitcsvm(transpose(X_train),transpose(Y_train)); 
    nn_model = fitcknn(transpose(X_train),transpose(Y_train)); 
    
    % Next, let's use our trained model to classify the examples in the 
    % test data. You should look up the function "predict" in Matlab!
    % (don't forget: transpose as necessary, both for X and Y)
    predictions_test_tree = predict(tree_model,transpose(X_test));
    predictions_test_svm = predict(svm_model,transpose(X_test));
    predictions_test_nn = predict(nn_model,transpose(X_test));
   
    % We can now proceed to computing the respective error rates.
    pred_test_diff_tree = predictions_test_tree - transpose(Y_test);
    pred_test_diff_svm = predictions_test_svm - transpose(Y_test); 
    pred_test_diff_nn = predictions_test_nn - transpose(Y_test); 
    ...
    predictions_train_tree = predict(tree_model,transpose(X_train));
    predictions_train_svm = predict(svm_model,transpose(X_train));
    predictions_train_nn = predict(nn_model,transpose(X_train));
   
    % We can now proceed to computing the respective error rates.
    pred_train_diff_tree = predictions_train_tree - transpose(Y_train);
    pred_train_diff_svm = predictions_train_svm - transpose(Y_train); 
    pred_train_diff_nn = predictions_train_nn - transpose(Y_train); 
