function [hitrate,confmat,allres,alljs,alljfg,allX,allY]=benchmark_inl4(mysystem,datadir,mode)
...
for ii=1:length(a)
    ...
    if strcmp(ext,'.jpg')
        ...
        clear f;
        for kk = 1:nrofsegments
            f(:,kk)=feval(mysystem.features,gtdata.S{kk});
        end
        ...
        if length(guess)<length(facit)
            guess = [guess '0'*ones(1,length(facit)-length(guess))];
        end
